...
print_help() {
	echo "usage: $0 docker/podman kill/nokill out_dir"
	echo "  docker/podman -- which container engine to use: docker or podman"
...
if [ "$RUNTIME" != "docker" -a "$RUNTIME" != "podman" ]; then
